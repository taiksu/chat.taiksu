...
class SettingsService {
  ...
  defaults() {
    return {
      ...
      aiAgentAvatar: String(process.env.AI_USER_AVATAR || '/images/system.png').trim() || '/images/system.png',
      ...
      aiTemperature: this.clampNumber(process.env.AI_TEMPERATURE, 0, 2, 0.35),
      aiMaxOutputTokens: this.clampInt(process.env.AI_MAX_OUTPUT_TOKENS, 64, 2048, 360),
      aiMaxReplyChars: this.clampInt(process.env.AI_MAX_REPLY_CHARS, 120, 4000, 520),
  ...
  save(input = {}) {
    ...
    const next = {
      ...
      aiAgentAvatar: input.aiAgentAvatar !== undefined
        ? (String(input.aiAgentAvatar || '').trim() || '/images/system.png')
        : String(current.aiAgentAvatar || '/images/system.png'),
      ...
      aiTemperature: input.aiTemperature !== undefined
        ? this.clampNumber(input.aiTemperature, 0, 2, this.clampNumber(current.aiTemperature, 0, 2, 0.35))
        : this.clampNumber(current.aiTemperature, 0, 2, 0.35),
      aiMaxOutputTokens: input.aiMaxOutputTokens !== undefined
        ? this.clampInt(input.aiMaxOutputTokens, 64, 2048, this.clampInt(current.aiMaxOutputTokens, 64, 2048, 360))
        : this.clampInt(current.aiMaxOutputTokens, 64, 2048, 360),
      aiMaxReplyChars: input.aiMaxReplyChars !== undefined
        ? this.clampInt(input.aiMaxReplyChars, 120, 4000, this.clampInt(current.aiMaxReplyChars, 120, 4000, 520))
        : this.clampInt(current.aiMaxReplyChars, 120, 4000, 520),
  ...
  safeForClient() {
    ...
    return {
      ...
      aiAgentAvatar: String(current.aiAgentAvatar || '/images/system.png'),
      aiPersonalityPrompt: String(current.aiPersonalityPrompt || ''),
      aiTemperature: this.clampNumber(current.aiTemperature, 0, 2, 0.35),
      aiMaxOutputTokens: this.clampInt(current.aiMaxOutputTokens, 64, 2048, 360),
      aiMaxReplyChars: this.clampInt(current.aiMaxReplyChars, 120, 4000, 520),
